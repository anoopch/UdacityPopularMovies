...
import ch.anoop.udacity.popularmovies.data.repository.MoviesRepository;
import ch.anoop.udacity.popularmovies.ui.activity.MovieDetailsActivity;
import ch.anoop.udacity.popularmovies.ui.helper.MoviesHelper;
import ch.anoop.udacity.popularmovies.ui.module.MoviesModule;
import ch.anoop.udacity.popularmovies.utils.Lists;
import ch.anoop.udacity.popularmovies.utils.UiUtils;
    ...
    private void onReviewsLoaded(List<Review> reviews) {
        ...
        if (!Lists.isEmpty(reviews)) {
            for (Review review : reviews) {
                ...
                getActivity().runOnUiThread(() -> mReviewsGroup.addView(reviewView));
                hasReviews = true;
            }
        }

        final boolean finalHasReviews = hasReviews;
        getActivity().runOnUiThread(() -> mReviewsGroup.setVisibility(finalHasReviews ? View.VISIBLE : View.GONE));
    }
    ...
    private void onVideosLoaded(List<Video> videos) {
        ...
        if (!Lists.isEmpty(videos)) {
            ...
            for (Video video : videos) {
                ...
                getActivity().runOnUiThread(() -> mVideosGroup.addView(videoView));
                hasVideos = true;
            }
        }
